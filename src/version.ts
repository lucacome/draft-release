...
export async function getVersionIncrease(latestRelease: string, major: string, minor: string, notes: string): Promise<string> {
    let majorTitle = '';
    let minorTitle = '';
    if (major !== '') {
    ...
    const version = await parseNotes(notes, majorTitle, minorTitle) as semver.ReleaseType;
